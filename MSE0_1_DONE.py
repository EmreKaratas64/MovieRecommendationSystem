...
# Load dataset (assuming MovieLens dataset format)
ratings = pd.read_csv('ratings.csv')
#movies = pd.read_csv('path_to_movies.csv')

# For the purpose of this example, we'll create some sample data
"""
data = {
    'userId': [1, 1, 1, 2, 2, 3, 3, 4, 4, 5],
    'movieId': [1, 2, 3, 1, 2, 2, 3, 1, 3, 2],
    'rating': [4, 5, 3, 5, 4, 4, 3, 2, 4, 5]
}
...
# Denormalize MSE for interpretation (if required)
denormalized_mse = mse * (ratings['rating'].max() - ratings['rating'].min()) ** 2
